...
function move(a){
	...
	var YOrigin = ($('#chicken').offset().top)
	var XOrigin = ($('#chicken').offset().left)
	...
	for (var i = 0; i < TotalDist; i += Math.PI/8) {
		...
		if (XOrigin + DistX < 0 || XOrigin + DistX > $(document).width() - 300)
			NewX = XOrigin - ((i/TotalDist)*DistX);
		else
			NewX = XOrigin + ((i/TotalDist)*DistX);
		if (YOrigin + DistY < 0 || YOrigin + DistY > $(document).height() - 300)
			NewY = YOrigin - ((i/TotalDist)*DistY) - (Math.abs(Math.sin(i))*20);
		else
			NewY = YOrigin + ((i/TotalDist)*DistY) - (Math.abs(Math.sin(i))*20); 
